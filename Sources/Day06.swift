...
struct Day06: AdventDay {
  ...
  func part1() -> Any {
    let values = lines.compactMap { $0.split(separator: ":").last }.map {
      $0.split { $0 == " " }.compactMap { Int($0) }
    }
    let times = values[0]
  ...
  func part2() -> Any {
    let values = lines.compactMap { $0.split(separator: ":").last?.filter { !$0.isWhitespace } }
      .compactMap { Int($0) }
    let time = values[0]
