...
struct Day01: AdventDay {
  ...
  func part1() -> Any {
    entities.map { Array($0).filter(\.isNumber) }.compactMap { Int("\($0[0])\($0[$0.count - 1])") }
      .reduce(0, +)
  }
  ...
  func part2() -> Any {
    ...
    return entities.map { line in
      ...
      for digit in digits {
        ...
        firstRange =
          firstRange.map { $0.lowerBound < ranges.first!.lowerBound ? $0 : ranges.first }
          ?? ranges.first
        lastRange =
          lastRange.map { $0.lowerBound > ranges.last!.lowerBound ? $0 : ranges.last }
          ?? ranges.last
      }
