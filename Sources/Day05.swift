...
struct Day05: AdventDay {
  ...
  init(data: String) {
    ...
    let values =
      lines.removeFirst().split(separator: ":").last?.split(separator: " ").compactMap { Int($0) }
      ?? []
    self.seeds = values
  ...
  private func location(for value: Int, mapIndex: Int) -> Int {
    ...
    return mapIndex == maps.count - 1
      ? destinationValue : location(for: destinationValue, mapIndex: mapIndex + 1)
  }
}
